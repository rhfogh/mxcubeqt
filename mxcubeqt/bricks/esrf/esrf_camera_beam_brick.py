...
class EsrfCameraBeamBrick(BaseWidget):
    def __init__(self, *args):
        ...
        self.ui_widgets_manager = qt_import.load_ui_file("camera_beam_brick.ui")
        ...
        # Wanted to connect directly HWR.beamline.config.sample_view signal to
        ...
        #     )
        # else:
        #     logging.getLogger("HWR").warning("CameraBeamBrick : HWR.beamline.config.sample_view NONE")

    # def beam_position_data_changed(self, new_beam_data):
    #     """
    #     auxiliar function to link HWR.beamline.config.sample_view to multipos_hwobj
    #     """        
    ...
    def beam_cal_pos_data_changed(self, who_changed, new_data_dict):
        ...
        for index_row in range(table.rowCount()):
            if table.item(index_row, 0).text() == pos_name:
                table.item(index_row, 1).setBackground(qt_import.QColor(qt_import.Qt.yellow))
                table.item(index_row, 2).setBackground(qt_import.QColor(qt_import.Qt.yellow))
            ...
            if not self.table_created:
                ...
                for row in range(len(tmp_dict)):
                    for col in range(3):
                        ...
                        tmp_item.setFlags(tmp_item.flags() ^ qt_import.Qt.ItemIsEditable)
