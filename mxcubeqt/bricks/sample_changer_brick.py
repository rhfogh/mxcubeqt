...
import mxcubeqt.utils.sample_changer_helper as sc_helper
...
class VialView(qt_import.QWidget):
    ...
    doubleClickSignal = qt_import.pyqtSignal(int)
    singleClickSignal = qt_import.pyqtSignal(int)
    ...
    def set_vial(self, vial_state):
        """Sets vial state"""
        if isinstance(vial_state, int):
            self.vial_state = vial_state
        else:
    ...
    def paintEvent(self, event):
        ...
        if self.vial_state is not None:
            ...
            painter.setBrush(qt_import.Qt.NoBrush)
    ...
    def mousePressEvent(self, event):
        ...
        qt_import.QWidget.mousePressEvent(self, event)

    def mouseReleaseEvent(self, event):
        """Mouse single clicked event"""
        self.singleClickSignal.emit(self.vial_index)
        qt_import.QWidget.mouseReleaseEvent(self, event)
...
class VialNumberView(qt_import.QLabel):

    doubleClickSignal = qt_import.pyqtSignal(int)
    singleClickSignal = qt_import.pyqtSignal(int)

    def __init__(self, vial_index, parent):
        qt_import.QWidget.__init__(self, str(vial_index), parent)
        self.vial_index = vial_index
    ...
    def mouseReleaseEvent(self, event):
        """Mouse single clicked event"""
        self.singleClickSignal.emit(self.vial_index)
        qt_import.QLabel.mouseReleaseEvent(self, event)
...
class SampleBox(qt_import.QWidget):
    ...
    def mouseMoveEvent(self, event):
        qt_import.QWidget.mouseMoveEvent(self, event)
    ...
    def enterEvent(self, event):
        qt_import.QWidget.enterEvent(self, event)
        if not self.selected:
            colors.set_widget_color(self, colors.LINE_EDIT_CHANGED)

    def leaveEvent(self, event):
        qt_import.QWidget.leaveEvent(self, event)
        if not self.selected:
            # set transparent color
            self.setStyleSheet("background-color: rgba(255,255,255,0);")


class SamplesView(qt_import.QWidget):

    CURRENT_VIAL_COLOR = qt_import.Qt.gray
    loadSampleSignal = qt_import.pyqtSignal(int, int)
    selectSampleSignal = qt_import.pyqtSignal(int, int)
    ...
    def __init__(self, parent, basket_index, no_samples=10, max_per_row=None):
        ...
        _main_hlayout = qt_import.QGridLayout(self)
        ...
        for index in range(self.no_samples):
            ...
            label.setAlignment(qt_import.Qt.AlignHCenter)
...
class BasketView(qt_import.QWidget):

    loadSampleSignal = qt_import.pyqtSignal(int, int)
    selectSampleSignal = qt_import.pyqtSignal(int, int)
    basketPresenceSignal = qt_import.pyqtSignal(int, bool)
    ...
    def __init__(
        ...
        self.contents_widget.setSizePolicy(
            qt_import.QSizePolicy.Minimum, qt_import.QSizePolicy.Minimum
        )
        self.setSizePolicy(
            qt_import.QSizePolicy.MinimumExpanding, qt_import.QSizePolicy.Fixed
        )
...
class CurrentView(qt_import.QWidget):
    def __init__(self, title, parent):
        qt_import.QWidget.__init__(self, parent)

        # self.standard_color=None
        self.standard_color = qt_import.Qt.white
        self.currentSelection = 1
        self.title = title

        self.contents_widget = qt_import.QGroupBox(title, self)
        self.contents_widget_hlayout = qt_import.QHBoxLayout(self.contents_widget)
        self.contents_widget_hlayout.setContentsMargins(0, 0, 0, 0)
        self.contents_widget_hlayout.setSpacing(0)
        self.contents_widget.setAlignment(qt_import.Qt.AlignLeft)
...
class CurrentBasketView(CurrentView):
    def __init__(self, parent):
        ...
        self.position_label = qt_import.QLabel("Position:", self.contents_widget)
        self.selected_spinbox = qt_import.QSpinBox(self.contents_widget)
        # self.selected_spinbox.setWrapping(True)
        # self.selected_spinbox.editor().setAlignment(QWidget.AlignRight)

        self.scan_basket_button = qt_import.QToolButton(self.contents_widget)
...
class CurrentSampleView(CurrentView):
    ...
    sampleChangerStateSignal = qt_import.pyqtSignal(str)
    ...
    def __init__(self, parent):
        ...
        _current_sample_view_widget = qt_import.QWidget(self.contents_widget)
        self.state_label = qt_import.QLabel("unknown", _current_sample_view_widget)
        self.state_label.setAlignment(qt_import.Qt.AlignHCenter)

        self.commands_widget = qt_import.QWidget(_current_sample_view_widget)
        self.position_label = qt_import.QLabel("Position:", self.commands_widget)
        self.selected_spinbox = qt_import.QSpinBox(self.commands_widget)
        self.selected_spinbox.setRange(1, 10)
        self.selected_spinbox.setSingleStep(1)
        self.selected_spinbox.setWrapping(True)
        self.selected_spinbox.lineEdit().setAlignment(qt_import.Qt.AlignRight)

        self.holder_length_label = qt_import.QLabel(
            "Holder length:", self.commands_widget
        )
        self.holder_length_spinbox = qt_import.QSpinBox(self.commands_widget)
        self.holder_length_spinbox.setRange(19, 26)
        self.holder_length_spinbox.setSingleStep(1)
        self.holder_length_spinbox.lineEdit().setAlignment(qt_import.Qt.AlignRight)
        self.holder_length_spinboxUnit = qt_import.QLabel("mm", self.commands_widget)
        ...
        self.load_button = qt_import.QToolButton(self.commands_widget)
        self.load_button.setText("Mount sample")
        # self.load_button.setTextPosition(QToolButton.BesideIcon)
        self.load_button.hide()

        _commands_widget_gridlayout = qt_import.QGridLayout(self.commands_widget)
        ...
        _current_sample_view_widget_vlayout = qt_import.QVBoxLayout(
...
class StatusView(qt_import.QWidget):
    ...
    def __init__(self, parent):
        ...
        # Graphic elements ----------------------------------------------------
        self.contents_widget = qt_import.QGroupBox("Unknown", self)
        ...
        self.reset_button = qt_import.QToolButton(self.box1)
        ...
        self.sc_can_load_radiobutton = qt_import.QRadioButton(
            "Sample changer can load/unload", self.contents_widget
        )
        self.minidiff_can_move_radiobutton = qt_import.QRadioButton(
...
class SCCheckBox(qt_import.QCheckBox):
...
class ScanBasketsView(qt_import.QWidget):
    def __init__(self, parent):
        qt_import.QWidget.__init__(self, parent)
        ...
        self.scan_all_baskets_button = qt_import.QToolButton(self)
        ...
        self.buttonSelect = qt_import.QToolButton(self)
...
class SampleChangerBrick(BaseWidget):
    ...
    sampleGotLoadedSignal = qt_import.pyqtSignal()
    ...
    def __init__(self, *args):
        ...
        self.contents_widget = qt_import.QWidget(self)
        self.status = self.build_status_view(self.contents_widget)
        self.switch_to_sample_transfer_button = qt_import.QPushButton(
            "Switch to Sample Transfer mode", self.contents_widget
        )
        self.test_sample_changer_button = qt_import.QPushButton(
        ...
        self.sc_contents_gbox = qt_import.QGroupBox("Contents", self)
        self.sc_contents_gbox.setAlignment(qt_import.Qt.AlignHCenter)
        self.reset_baskets_samples_button = qt_import.QPushButton(
        ...
        self.operations_widget = qt_import.QWidget(self)
        self.build_operations_widget()

        self.baskets_grid_layout = qt_import.QGridLayout()
        ...
        self.sc_contents_gbox_vlayout = qt_import.QVBoxLayout(self.sc_contents_gbox)
        ...
        if property_name == "icons":
            ...
            if self.basket_count is not None and self.basket_label is not None:
             ...
             if HWR.beamline.config.sample_changer is not None:
                ...
                self.sc_status_changed(HWR.beamline.config.sample_changer.get_status())
                self.sc_state_changed(HWR.beamline.config.sample_changer.get_state())
                self.infoChanged()
                self.selectionChanged()
                self.loadedSampleChanged(HWR.beamline.config.sample_changer.get_loaded_sample())
    ...
    def selectionChanged(self):
        sample = HWR.beamline.config.sample_changer.get_selected_sample()
        basket = HWR.beamline.config.sample_changer.get_selected_component()
        if sample is None:
    ...
    def switchToSampleTransferMode(self):
        HWR.beamline.config.sample_changer.change_mode(
    ...
    def resetSampleChanger(self):
        HWR.beamline.config.sample_changer.reset()

    def resetBasketsSamplesInfo(self):
        HWR.beamline.config.sample_changer.clear_info()
    ...
    def run(self):
        ...
        try:
            self.matrixCodesChanged(HWR.beamline.config.sample_changer.get_matrix_codes())
        except BaseException:
            pass
    ...
    def sampleChangerToLoadingPosition(self):
        if not HWR.beamline.config.sample_changer.sample_changer_to_loading_position():
            self.status.setSampleChangerLoadStatus(
                HWR.beamline.config.sample_changer.sample_changer_can_load()
            )

    def minidiffGetControl(self):
        if not HWR.beamline.config.sample_changer.minidiffGetControl():
            self.status.setMinidiffStatus(HWR.beamline.config.sample_changer.minidiffCanMove())

    def changeBasket(self, basket_number):
        address = Container.Basket.get_basket_address(basket_number)
        HWR.beamline.config.sample_changer.select(address, wait=False)

    def changeSample(self, sample_number):
        basket_index = HWR.beamline.config.sample_changer.getSelectedComponent().get_index()
    ...
    def load_this_sample(self, basket_index, vial_index):
        if self.double_click_loads_cbox.isChecked():
            ...
            HWR.beamline.config.sample_changer.load((basket_index, vial_index), wait=False)

    def loadSample(self, holder_len):
        HWR.beamline.config.sample_changer.load(
            holder_len,
            None,
            None,
    ...
    def scanBasket(self):
        ...
        HWR.beamline.config.sample_changer.scan(
            HWR.beamline.config.sample_changer.getSelectedComponent(),
            recursive=True,
            wait=False
        )
    ...
    def infoChanged(self):
        baskets_at_sc = HWR.beamline.config.sample_changer.get_components()
        presences = []
        ...
        for sample in HWR.beamline.config.sample_changer.get_sample_list():
    ...
    def select_baskets_samples(self):
        ...
        if retval == qt_import.QDialog.Accepted:
            HWR.beamline.config.sample_changer.reset_baskets_information()
            ...
            for (basket, samples) in self.basketsSamplesSelectionDialog.result.items():
                ...
                for sample in samples:
                    ...
                    basket_input[2] = 1
                    HWR.beamline.config.sample_changer.set_basket_sample_information(basket_input)

    def test_sample_changer(self):
        HWR.beamline.config.sample_changer.run_test()
