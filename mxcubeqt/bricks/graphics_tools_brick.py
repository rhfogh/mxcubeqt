...
class GraphicsToolsBrick(BaseWidget):
    ...
    def run(self):
        self.tools_menu = qt_import.QMenu("Graphics", self)
    ...
    def init_image_scale_list(self):
        self.image_scale_list = HWR.beamline.config.sample_view.get_image_scale_list()
        if len(self.image_scale_list) > 0:
            ...
            self.image_scaled(HWR.beamline.config.sample_view.get_image_scale())
    ...
    def image_scale_triggered(self, selected_action):
        for index, action in enumerate(self.image_scale_menu.actions()):
            if selected_action == action:
                HWR.beamline.config.sample_view.set_image_scale(
    ...
    def measure_distance_clicked(self):
        HWR.beamline.config.sample_view.start_measure_distance(wait_click=True)

    def measure_angle_clicked(self):
        HWR.beamline.config.sample_view.start_measure_angle(wait_click=True)

    def measure_area_clicked(self):
        HWR.beamline.config.sample_view.start_measure_area(wait_click=True)
    ...
    def create_point_current_clicked(self):
        if self.isEnabled():
            HWR.beamline.config.sample_view.start_centring()
