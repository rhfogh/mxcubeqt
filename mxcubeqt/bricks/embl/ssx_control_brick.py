...
class SsxControlBrick(BaseWidget):
    def __init__(self, *args):
        ...
        # Graphic elements ----------------------------------------------------
        self.ssx_widget_layout = qt_import.load_ui_file("ssx_control_widget_layout.ui")
        ...
        for row in range(0, 4):
            ...
            checkbox_item.setFlags(
                qt_import.Qt.ItemIsUserCheckable | qt_import.Qt.ItemIsEnabled
            )
    ...
    def get_chip_file_directory(self):
        self.chip_file_dir = qt_import.QFileDialog.getExistingDirectory(
    ...
    def update_found_chip_files(self):
        ...
        self.chip_files_listwidget = qt_import.QListWidget(
    ...
    def get_shortlist_directory(self):
        self.shortlist_dir = qt_import.QFileDialog.getExistingDirectory(
            self, "Select chip files folder"
        )
    ...
    def update_table(self):
        ...
        for element in range(0, len(interlacingsList)):
            self.ssx_widget_layout.interlacings_tableWidget.setItem(
                element, 0, qt_import.QTableWidgetItem(str(interlacingsList[element]))
            )
            self.ssx_widget_layout.interlacings_tableWidget.setItem(
                element,
                1,
    ...
    def create_dg_channels_list(self, num_exp):
        ...
        if num_exp > chip_config["old_num_of_exp"]:
            for row in range(0, 4):
                checkbox_item = qt_import.QTableWidgetItem()
                checkbox_item.setFlags(
                    qt_import.Qt.ItemIsUserCheckable | QtCore.Qt.ItemIsEnabled
                )
                checkbox_item.setCheckState(qt_import.Qt.Unchecked)
    ...
    def save_chip_data(self):
        ...
        new_chip_filename, ok = qt_import.QInputDialog.getText(
    ...
    def export_short_list(self):
        # opens input dialod to enter file_name
        new_shortlist_name, ok = qt_import.QInputDialog.getText(
