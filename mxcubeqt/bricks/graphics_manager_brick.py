...
class GraphicsManagerBrick(BaseWidget):
    def __init__(self, *args):
        ...
        self.manager_widget = qt_import.load_ui_file("graphics_manager_layout.ui")
        ...
        self.connect(HWR.beamline.config.sample_view, "shapeCreated", self.shape_created)
        self.connect(HWR.beamline.config.sample_view, "shapeDeleted", self.shape_deleted)
        self.connect(HWR.beamline.config.sample_view, "shapeSelected", self.shape_selected)
        self.connect(
    ...
    def shape_selected(self, shape, selected_state):
        if shape in self.__shape_map:
            self.__shape_map[shape].setData(
                4, qt_import.Qt.DisplayRole, str(selected_state)
            )
            ...
            self.manager_widget.change_color_button.setEnabled(
                bool(HWR.beamline.config.sample_view.get_selected_shapes())
            )
    ...
    def change_color_clicked(self):
        ...
        if color.isValid():
            for item in HWR.beamline.config.sample_view.get_selected_shapes():
                item.set_base_color(color)

    def display_all_button_clicked(self):
        for shape, treewidget_item in self.__shape_map.items():
            shape.show()
            treewidget_item.setData(3, qt_import.Qt.DisplayRole, "True")

    def hide_all_button_clicked(self):
        for shape, treewidget_item in self.__shape_map.items():
            shape.hide()
            treewidget_item.setData(3, qt_import.Qt.DisplayRole, "False")
    ...
    def draw_grid_button_clicked(self):
        HWR.beamline.config.sample_view.create_grid(self.get_spacing())

    def show_shape_treewidget_popup(self, item, point, col):
        qt_import.QMenu(self.manager_widget.shapes_treewidget)
