...
class AlbaMachineInfoBrick(BaseWidget):
    ...
    def __init__(self, *args):
        ...
        self.logger = logging.getLogger("GUI ALBA MachInfo")
        self.logger.info("ALBAMachineInfoBrick.__init__()")
        # Hardware objects ----------------------------------------------------
    ...
    def property_changed(self, property_name, old_value, new_value):
        ...
        if property_name == 'mnemonic':
            if self.mach_info_hwobj is not None:
                self.disconnect(self.mach_info_hwobj, qt_import.SIGNAL('valuesChanged'),
                                self.set_value)

            self.mach_info_hwobj = self.get_hardware_object(new_value)
            if self.mach_info_hwobj is not None:
                self.setEnabled(True)
                self.connect(self.mach_info_hwobj, qt_import.SIGNAL('valuesChanged'),
                             self.set_value)
