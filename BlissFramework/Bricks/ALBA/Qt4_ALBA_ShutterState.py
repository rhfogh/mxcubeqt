...
class Qt4_ALBA_ShuttersBrick(BlissWidget):
    ...
    def propertyChanged(self, property_name, old_value, new_value):
        ...
        if property_name == 'fast_shutter':
            ...
            if self.fast_shut_ho is not None:
                self.logger.debug("Connecting %s" % self.fast_shut_ho.getUserName())
            ...
            if self.slow_shut_ho is not None:
                self.logger.debug("Connecting %s" % self.slow_shut_ho.getUserName())
            ...
            if self.photon_shut_ho is not None:
                self.logger.debug("Connecting %s" % self.photon_shut_ho.getUserName())
            ...
            if self.fe_ho is not None:
                self.logger.debug("Connecting %s" % self.fe_ho.getUserName())
