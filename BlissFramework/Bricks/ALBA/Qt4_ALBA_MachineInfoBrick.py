...
class Qt4_ALBA_MachineInfoBrick(BlissWidget):
    ...
    def __init__(self, *args):
        ...
        self.logger.info("ALBA_MachineInfoBrick.__init__()")
        # Hardware objects ----------------------------------------------------
        ...
        # Graphic elements ----------------------------------------------------
        self.groupBox = QGroupBox("Machine Info", self)
        ...
        self.current_label = QLabel("Machine current:")
        ...
        self.state_text_label = QLabel("Machine status:")
        self.state_text_value_label = QLabel()
        self.state_text_value_label.setAlignment(Qt.AlignCenter)
        self.state_text_value_label.setStyleSheet("font-size: 18px;")

        self.topup_remaining_label = QLabel("TopUp Remaining:")
        self.topup_remaining_value_label = QLabel()
        self.topup_remaining_value_label.setAlignment(Qt.AlignCenter)
        ...
        # Other --------------------------------------------------------------- 
        self.setToolTip("Main information about machine current, machine status and "
                        "top-up remaining time.")
    ...
    def propertyChanged(self, property_name, old_value, new_value):
        ...
        if property_name == 'mnemonic':
            if self.mach_info_hwobj is not None:
                self.disconnect(self.mach_info_hwobj, SIGNAL('valuesChanged'),
                                self.set_value)

            self.mach_info_hwobj = self.getHardwareObject(new_value)
            if self.mach_info_hwobj is not None:
                self.setEnabled(True)
                self.connect(self.mach_info_hwobj, SIGNAL('valuesChanged'),
                             self.set_value)
    ...
    def set_value(self, values):
        ...
        if values[2] != '':
            # self.logger.debug("Set top up remaining to %s (type %s)", values[2],
            #                   type(values[2]))
            txt = '??? s' if values[2] is None else '%s s' % str(self['formatString']
                                                                 % values[2])
